...
def find_llama_binaries(llama_bin: str) -> dict:
    """
    Newer llama.cpp builds ship multiple binaries.
    llama-run is single-shot by design; llama-cli is interactive by default.
    Returns dict of available binaries in the same directory.
    """
    bin_dir = Path(llama_bin).parent
    candidates = {
        "run":  bin_dir / "llama-run",    # single-shot, best for batch
        "cli":  bin_dir / "llama-cli",    # interactive, needs --no-cnv
        "main": bin_dir / "main",         # old name, pre-rename
    }
...
def run_inference(llama_bin: str, model_path: str, prompt: str,
    ...
    Handles multiple llama.cpp build generations:
      - Old builds: llama-cli with -no-cnv (single dash)
      - Mid builds: llama-cli with --no-cnv (double dash)
      - New builds (b9000+): use llama-run for single-shot inference
    """
    bins = find_llama_binaries(llama_bin)

    gpu_before = get_gpu_stats()

    # --- Strategy 1: llama-run (cleanest, purpose-built for single-shot) ---
    if "run" in bins:
        cmd = [
            bins["run"],
        ...
        try:
            ...
            if result.returncode == 0:
                return _parse_result(result, prompt, elapsed, gpu_before)
        except subprocess.TimeoutExpired:
            return {"success": False, "error": "timeout", "elapsed_sec": 300}

    # --- Strategy 2: llama-cli with exit flags (--single-turn for b9481+, --no-cnv for older) ---
    cli_bin = bins.get("cli", bins.get("main", llama_bin))
    for exit_flag in ["--single-turn", "--no-cnv", "-no-cnv"]:
        cmd = [
            cli_bin,
            "-m",             model_path,
            "-p",             prompt,
        ...
        try:
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=300)
            elapsed = round(time.perf_counter() - t_start, 3)
            # Accept if it worked; skip if flag was rejected
            if result.returncode == 0:
                return _parse_result(result, prompt, elapsed, gpu_before)
            rejected = any(p in result.stderr.lower()
                           for p in ("unknown", "invalid argument", "unrecognized"))
            if not rejected:
                # Failed for another reason — return the error
                return {"success": False, "error": result.stderr[-400:], "elapsed_sec": elapsed}
            # else: flag rejected, try next flag variant
        except subprocess.TimeoutExpired:
            return {"success": False, "error": "timeout", "elapsed_sec": 300}

    # --- Strategy 3: stdin pipe with explicit EOF (last resort) ---
    # Works on interactive builds: sends the prompt then closes stdin so the
    # process sees EOF and exits cleanly after generation.
    cmd = [
        cli_bin,
        "-m",             model_path,
        "--n-predict",    str(n_predict),
    ...
    try:
        result = subprocess.run(
            cmd,
            input=prompt,       # closes stdin (EOF) after sending prompt
            capture_output=True,
            text=True,
            timeout=300,
        )
        ...
        return {"success": False, "error": "timeout (stdin)", "elapsed_sec": 300}
    except Exception as e:
        return {"success": False, "error": str(e), "elapsed_sec": 0}


def _parse_result(result: subprocess.CompletedProcess, prompt: str,
                  elapsed: float, gpu_before: dict) -> dict:
    """Extract generated text and timing stats from a successful llama.cpp run."""
    full_output = result.stdout.strip()

    # Strip echoed prompt to isolate generated text
    if "[/INST]" in full_output:
        generated = full_output.split("[/INST]")[-1].strip()
    elif prompt.strip() in full_output:
        generated = full_output.replace(prompt.strip(), "").strip()
    else:
    ...
    # New: "[ Prompt: X t/s | Generation: Y t/s ]"  (in stdout)
    # Old: "eval time = ... (X tokens per second)"   (in stderr)
    tokens_per_sec = None
    for line in (result.stdout + result.stderr).split("\n"):
        if "Generation:" in line and "t/s" in line:
            try:
                tokens_per_sec = float(line.split("Generation:")[-1].split("t/s")[0].strip())
                break
            except Exception:
                pass
